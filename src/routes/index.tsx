      ...
      <main className="relative z-10 mx-auto max-w-6xl px-5 pb-24 pt-16 sm:px-8 sm:pt-24">
        <header className="animate-fade-up text-center">
          ...
          <h1 className="mt-8 font-serif text-5xl leading-[1.05] tracking-tight text-foreground sm:text-7xl">
            {t("heroTitle1")} <span className="text-gradient-gold italic">{t("heroTitleAccent")}</span>
            <br /> {t("heroTitle2")}
          </h1>
...
interface HomeCardProps {
  to: string; number: string; label: string; title: string; description: string; cta: string; icon: React.ReactNode; delay: number;
}

function HomeCard({ to, number, label, title, description, cta, icon, delay }: HomeCardProps) {
  return (
    <Link
      to={to as any}
    ...
    >
      <div
        className="absolute -end-20 -top-20 h-64 w-64 rounded-full opacity-0 transition duration-700 group-hover:opacity-100"
